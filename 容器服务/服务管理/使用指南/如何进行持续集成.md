...
挂载目录填写/var/jenkins_home/，挂载到这个路径的原因是 jenkins 的配置信息在路径/var/jenkins_home/下，这样 jenkins 的配置数据便存储在了云硬盘中，后期需要重新建 master 节点时直接将之前的云硬盘挂载到路径/var/jenkins_home/即可，这样可以节省 jenkins 的配置操作。如图:
...
点击「高级设置」，此处需要添加两个环境变量，JNLP_CRENDIENTIALS  jenkins:jenkins 和 JNLP_URL http://59.111.96.140:8080/computer/slave/slave-agent.jnlp ，jenkins:jenkins指的是用于jenkins连接的用户名和密码，由于jenkins是管理员权限，考虑到安全性此处建议使用专门用于jenkins连接的用户，如前面新建的用户ciwebhook，JNLP_URL的值为新建子节点时记录的jnlpUrl的值，如图:
