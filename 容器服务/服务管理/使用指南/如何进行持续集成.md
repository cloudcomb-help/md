...
为区别于其他服务建议在蜂巢中新建一个空间，如 ci，如图所示:
![](../image/namespace.png)
...
进入管理节点之后点击「新建节点」，填写节点名称，如salve，如图:
...
点击「高级设置」，此处需要添加两个环境变量，JNLP_CRENDIENTIALS  jenkins:jenkins 和 JNLP_URL http://59.111.96.140:8080/computer/slave/slave-agent.jnlp ，jenkins:jenkins指的是用于jenkins连接的用户名和密码，由于jenkins是管理员权限，考虑到安全性此处建议新建专门用于jenkins连接的用户，如前面新建的用户ciwebhook，JNLP_URL的值为新建子节点时记录的jnlpUrl的值，如图:
...
填写远程「工作目录」，如"/home/jenkins/workspace/"，选择「用法」,如"只允许运行绑定到这台机器的job"，选择「启动方法」，如"Launch slave agents via ssh"，其他选项默认即可，如图:
