...
<span>Note:</span><div class="alertContent">请注意参数之间的逻辑关系，如无状态服务不需要绑定公网。</div>
...
| namespace_id                | long             | 是       | 空间 id（[获取空间列表](http://59.111.120.124/?http#9-2)）                                                                                            | 51616                                |
...
| spec_alias                  | String           | 是       | 规格别名（[规格参数](http://59.111.120.124/?http#9-0)）                                                                                               | C1M2S20                              |
...
| ip_id                       | Stirng           | 否       | IP 的 uuid（[获取 IP 列表](http://59.111.120.124/?http#8-2-ip)）                                                                                      | cef9069c-d97d-42b1-a86c-f815f748820c |
| **service_container_infos** | jsonArray        | 是       | 容器相关元信息                                                                                                                     | 详见示例                             |
| image_path                  | String           | 是       | 镜像地址（[获取镜像列表](http://59.111.120.124/?http#9-5)）                                                                                           | hub.c.163.com/public/ubuntu:14.04    |
...
| ssh_keys                    | List             | 否       | 需要注入的 ssh 密钥名称（[获取密钥列表](http://59.111.120.124/?http#4-2)）                                                                            | ["sshkey1","sshkey2"]                |
| volume_info                 | Map<Long,String> | 否       | 云硬盘挂载信息{云盘id:挂载路径}（[获取云硬盘列表](http://59.111.120.124/?http#7-2)）                                                                  | {"163": "/mnt/"}                     |
