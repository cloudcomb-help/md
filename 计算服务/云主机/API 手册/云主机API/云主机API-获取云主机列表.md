  ...
  "instances": [
    {
        ...
        "status": "ACTIVE", #云主机状态
        "dhcp": 1, #是否ip为自动分配
        "vnet_ip": "10.18.192.43", #vnet ip地址
        "root_gb": 20, #系统盘容量
        "vcpu": 4, #CPU 核数
        "memory_gb": 4, #内存大小
        "flavorId": 170, #云主机规格id
        "loginTimes": 0, #云主机登录次数
        "billCategory": 1, #计费类型
        "vnet_port_id": "e55f0351-4d8f-4a44-8f5d-9cccaa261a1e", #云主机网络端口
        ...
        "public_ip": null, #云主机绑定的 IP 地址
        "public_port_id": null, #云主机绑定IP 地址的端口
        "bandWidth": null, #绑定公网ip的带宽
        "public_port_status": null, #绑定公网ip状态
        "publicQosMode": null, #绑定公网ip计费形式(按带宽计费还是流量)
...
| dhcp             | Integer    | ？                                        | 1                                    |
| vnet_ip          | String     | ？     vnet ip地址                        | 10.18.192.43                         |
| root_gb          | Integer    | 系统盘容量，单位 GB                       | 20                                   |
| vcpu             | Integer    | CPU 核数                                  | 4                                    |
| memory_gb        | Integer    | 内存大小                                  | 4                                    |
| flavorId         | Integer    | 规格 id                                   | 170                                  |
| loginTimes       | Integer    | ？云主机登录次数                          | 0                                    |
| billCategory     | Integer    | ？计费类型                                | 1                                    |
| vnet_port_id     | String     | ？网络端口                                | e55f0351-4d8f-4a44-8f5d-9cccaa261a1e |
...
| public_ip        | String     | 云主机绑定的 IP 地址（为空表示未绑定）    | null                                 |
| public_port_id   | ？Integer  | 云主机绑定IP 地址的端口（为空表示未绑定） | null                                 |
| publicQosMode    | String     | ？云主机绑定的 IP 地址计费类型            | null                                 |
| properties       | ？         | 云主机秘钥                                | ？{\"publicKeys\":\"ubuntu,\"}       |
